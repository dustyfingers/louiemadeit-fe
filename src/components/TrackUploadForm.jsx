...
import {
    ...
    setTrackCoverArt,
    setTrackStems,
} from '../redux/admin/track/track-actions';
...
const TrackUploadForm = ({
    ...
    const handleUploadToS3 = async (file, putUrl) => {
        ...
        await axios.put(urlResponse.data.putUrl, file, {
        ...
        >
            <div>
                <div id="textSection" className="mt-2 mb-4">
                    ...
                    <div>
                        <label htmlFor="trackDescription" className="form-label">
                            Description:
                        </label>
                        <input
                            type="text"
                            ...
                            onChange={evt =>
                                dispatch(setTrackDescription(evt.target.value))
                            }
                            disabled={uploading ? true : false}
                        />
                    </div>
                ...
                <div id="filesSection" className="mb-4">
                    <div className="mb-2">
                        <label htmlFor="taggedVersion" className="form-label">
                            Tagged Version:
                        </label>
                        <input
                            ...
                            accept="audio/*"
                            onChange={evt =>
                                dispatch(setTrackTaggedVersion(evt.target.files))
                            }
                            disabled={uploading ? true : false}
                        />
                    </div>
                    ...
                    <div className="mb-2">
                        <label htmlFor="untaggedVersion" className="form-label">
                            Untagged Version:
                        </label>
                        <input
                            className="form-control"
                            ...
                            accept="audio/*"
                            onChange={evt =>
                                dispatch(setTrackUntaggedVersion(evt.target.files))
                            }
                            disabled={uploading ? true : false}
                        />
                    </div>
...
const mapStateToProps = state => ({
    ...
    sellType: state.track.sellType,
    exclusivePrice: state.track.exclusivePrice,
    leaseStemsPrice: state.track.leaseStemsPrice,
    leaseMasterOnlyPrice: state.track.leaseMasterOnlyPrice,
    taggedVersion: state.track.taggedVersion,
    untaggedVersion: state.track.untaggedVersion,
    coverArt: state.track.coverArt,
    trackStems: state.track.trackStems,
});
